...
st.set_page_config(
    page_title="Market Dashboard -Sector Index BOF", # Browser tab title
    page_icon="📊", # Optional: Emoji or path to icon
    layout="wide"
)

st.title("📊 Sector Wise Stock BOF  Dashboard")
...
if st.button("Run Fractal Data Fetch"):
    st.write("Running data fetch...")
    subprocess.run(["python3", base_dir + "sector_wise_datafetch.py"])
...
# with col2:
#     with st.container(border=True):
#         st.subheader("VRZ High")
#         st.title(":red[2]")
#
# with col3:
#     with st.container(border=True):
#         st.subheader("VRZ Low")
#         st.title(":green[8]")
...
sector_files = {
    "Auto": base_dir + "niftyauto_fractal_levels_1day.csv",
    "FMCG": base_dir + "niftyfmcg_fractal_levels_1day.csv",
    "IT": base_dir + "niftyit_fractal_levels_1day.csv",
}
...
for sector, file in sector_files.items():
    try:
        ...
        st.write(f"{sector} data not available")
